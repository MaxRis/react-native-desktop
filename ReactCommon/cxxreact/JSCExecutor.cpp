...
template <JSValueRef (JSCExecutor::*method)(JSObjectRef object,
                                            JSStringRef propertyName)>
inline JSObjectGetPropertyCallback exceptionWrapMethod() {
  struct funcWrapper {
    static JSValueRef call(JSContextRef ctx, JSObjectRef object,
...
#if DEBUG
static JSValueRef nativeInjectHMRUpdate(JSContextRef ctx, JSObjectRef function,
...
void JSCExecutor::initOnJSVMThread() throw(JSException) {
  ...
  if (canUseInspector(m_context)) {
    ...
    JSC_JSGlobalContextEnableDebugger(m_context, globalInspector,
                                      ownerId.c_str(), checkIsInspectedRemote);
  }
  ...
  installGlobalFunction(m_context, "nativeLoggingHook",
                        JSCNativeHooks::loggingHook);
  installGlobalFunction(m_context, "nativePerformanceNow",
                        JSCNativeHooks::nowHook);*/

#if DEBUG
  installGlobalFunction(m_context, "nativeInjectHMRUpdate",
...
bool JSCExecutor::isNetworkInspected(const std::string &owner,
  ...
  auto connect_socket = [](int socket_desc, std::string address, int port) {
    ...
    tv.tv_sec = 1;
    tv.tv_usec = 0;
    auto sock_opt_rcv_resp =
        setsockopt(socket_desc, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv,
    ...
    auto sock_opt_snd_resp =
        setsockopt(socket_desc, SOL_SOCKET, SO_SNDTIMEO, (const char *)&tv,
                   sizeof(struct timeval));
  ...
  std::string msg = folly::to<std::string>(
      "GET /autoattach?title=", escapedOwner, "&app=", escapedApp, "&device=",
      escapedDevice, " HTTP/1.1\r\n\r\n");
...
void JSCExecutor::loadApplicationScript(
  ...
  // ReactMarker::logTaggedMarker(ReactMarker::RUN_JS_BUNDLE_START,
  //                             scriptName.c_str());
...
// TODO t15069155: reduce the number of overrides here
  ...
  if (auto fileStr = dynamic_cast<const JSBigFileString *>(script.get())) {
    ...
    auto bcSourceCode = JSCreateSourceCodeFromFile(fileStr->fd(), jsSourceURL,
                                                   nullptr, &jsStatus);
    ...
    case JSLoadSourceIsCompiled:
      ...
      ReactMarker::logTaggedMarker(ReactMarker::RUN_JS_BUNDLE_STOP,
                                   scriptName.c_str());
      return;
  ...
  BundleHeader header;
  memcpy(&header, script->c_str(),
         std::min(script->size(), sizeof(BundleHeader)));
  ...
  if (scriptTag == ScriptTag::BCBundle) {
    ...
    JSValueRef jsError;
    JSValueRef result = JSC_JSEvaluateBytecodeBundle(m_context, NULL, sourceFD,
                                                     jsSourceURL, &jsError);
    if (result == nullptr) {
...
void JSCExecutor::registerBundle(uint32_t bundleId,
...
void JSCExecutor::bindBridge() throw(JSException) {
  ...
  std::call_once(m_bindFlag, [this] {
    ...
    if (batchedBridgeValue.isUndefined()) {
      ...
      if (batchedBridgeValue.isUndefined()) {
        throw JSException("Could not get BatchedBridge, make sure your bundle "
...
Value JSCExecutor::callFunctionSyncWithValue(const std::string &module,
  ...
  Object result = [&] {
    ...
    return m_callFunctionReturnResultAndFlushedQueueJS
        ->callAsFunction({
...
void JSCExecutor::setGlobalVariable(
  ...
  try {
    ...
    Object::getGlobalObject(m_context).setProperty(propName.c_str(),
                                                   valueToInject);
  } catch (...) {
    std::throw_with_nested(
...
String JSCExecutor::adoptString(std::unique_ptr<const JSBigString> script) {
  ...
  auto jsString = JSStringCreateAdoptingExternal(
      string->c_str(), string->size(), (void *)string,
      [](void *s) { delete static_cast<JSBigString *>(s); });
  return String::adopt(m_context, jsString);
#else
  return script->isAscii() ? String::createExpectingAscii(
                                 m_context, script->c_str(), script->size())
                           : String(m_context, script->c_str());
#endif
}

void *JSCExecutor::getJavaScriptContext() { return m_context; }

bool JSCExecutor::isInspectable() { return canUseInspector(m_context); }

#ifdef WITH_JSC_MEMORY_PRESSURE
void JSCExecutor::handleMemoryPressure(int pressureLevel) {
  JSHandleMemoryPressure(this, m_context,
                         static_cast<JSMemoryPressure>(pressureLevel));
}
#endif
...
JSValueRef JSCExecutor::getNativeModule(JSObjectRef object,
                                        JSStringRef propertyName) {
...
JSValueRef JSCExecutor::nativeRequire(size_t argumentCount,
                                      const JSValueRef arguments[]) {
  uint32_t bundleId, moduleId;
  std::tie(bundleId, moduleId) =
      parseNativeRequireParameters(m_context, arguments, argumentCount);
...
JSValueRef
JSCExecutor::nativeFlushQueueImmediate(size_t argumentCount,
                                       const JSValueRef arguments[]) {
  if (argumentCount != 1) {
...
JSValueRef JSCExecutor::nativeCallSyncHook(size_t argumentCount,
                                           const JSValueRef arguments[]) {
  if (argumentCount != 3) {
    throw std::invalid_argument("Got wrong number of args");
  }
